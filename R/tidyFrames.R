...
tidyFrames <- function(metric) {
    if (metric == "income") {
        ...
        allWeightedIncome <- Hmisc::Merge(ownerWeightedIncome, renterWeightedIncome, 
            id = ~someyears)
        ...
        renterWeightedConsumption <- censusFunctions::weighHouseholds("renters", 
            "consumption")
        ownerWeightedConsumption <- censusFunctions::weighHouseholds("owners", "consumption")
        someyears <- c(2004:2014)
        allWeightedConsumption <- Hmisc::Merge(ownerWeightedConsumption, renterWeightedConsumption, 
            id = ~someyears)
        ...
        allWeightedNHC <- Hmisc::Merge(ownerWeightedNHC, renterWeightedNHC, id = ~someyears)
        ...
        return("only metrics income/consumption/NHC")
    }
}
