...
renameColumns <- function(arg = TRUE) {
    ...
    for (df in realList) {
        df.tmp <- get(df)
        names(df.tmp) <- c("grossIncome", "rentalIncome", "totalConsumption", "rentEXP", 
            "imputedRent", "WEIGHT", "HHNUM")
        assign(df, df.tmp)
    }
    ## 
    years <- c(2004:2014)
    dfs <- list(exp2004s, exp2005s, exp2006s, exp2007s, exp2008s, exp2009s, exp2010s, 
        exp2011s, exp2012s, exp2013s, exp2014s)
